# -*- coding: utf-8 -*-
"""
Created on Thu Sep 26 09:32:28 2019
...
output_folder = './image/val_256_denoise_Gaussian_3_Unet'
batch_size = 16
eval_size = 16
img_height = 256
img_width = 256
...
#    W =tf.Variable(tf.zeros([3,3,3,64]),trainable=True,name = 'weight1_1')
    net['conv1_1'] = tf.layers.conv2d(net['input'], filters = 32, kernel_size = [3,3],padding = 'SAME', activation=tf.nn.relu,use_bias=True,bias_initializer=tf.zeros_initializer())
#    net['crop1_1'] = tf.keras.layers.Cropping2D(cropping=((img_height*1/8,img_height*1/8),(img_width*1/8,img_width*1/8)))(net['conv1_1'])
    net['pool1_1'] = tf.nn.avg_pool(net['conv1_1'],[1,2,2,1],strides=[1,2,2,1],padding='SAME',name='pool2_1')
    net['conv1_2'] = tf.layers.conv2d(net['pool1_1'], filters = 64, kernel_size = [3,3],padding = 'SAME', activation=tf.nn.relu,use_bias=True,bias_initializer=tf.zeros_initializer())
#    net['crop1_2'] = tf.keras.layers.Cropping2D(cropping=((img_height*1/8/2,img_height*1/8/2),(img_width*1/8/2,img_width*1/8/2)))(net['conv1_2'])
    net['pool1_2'] = tf.nn.avg_pool(net['conv1_2'],[1,2,2,1],strides=[1,2,2,1],padding='SAME',name='pool2_1')
    net['conv1_3'] = tf.layers.conv2d(net['pool1_2'], filters = 128, kernel_size = [3,3],padding = 'SAME', activation=tf.nn.relu,use_bias=True,bias_initializer=tf.zeros_initializer())
#    net['crop1_1'] = tf.keras.layers.Cropping2D(cropping=((img_height*1/8/4,img_height*1/8/4),(img_width*1/8/4,img_width*1/8/4)))(net['conv1_1'])
    net['pool1_3'] = tf.nn.avg_pool(net['conv1_3'],[1,2,2,1],strides=[1,2,2,1],padding='SAME',name='pool2_1')
    net['conv1_4'] = tf.layers.conv2d(net['pool1_3'], filters = 256, kernel_size = [3,3],padding = 'SAME', activation=tf.nn.relu,use_bias=True,bias_initializer=tf.zeros_initializer())
    
    print('LAYER GROUP 2')
    net['un_pool_2_1'] = tf.compat.v1.keras.layers.UpSampling2D(size=(2,2),data_format=None,interpolation='nearest')(net['conv1_4'])
    net['conv2_1'] = tf.layers.conv2d(net['un_pool_2_1'], filters = 128, kernel_size = [3,3],padding = 'SAME', activation=tf.nn.relu,use_bias=True,bias_initializer=tf.zeros_initializer())
    net['concat2_1'] = tf.compat.v1.concat([net['conv1_3'],net['conv2_1']],3)
    
    net['un_pool_2_2'] = tf.compat.v1.keras.layers.UpSampling2D(size=(2,2),data_format=None,interpolation='nearest')(net['concat2_1'])
    net['conv2_2'] = tf.layers.conv2d(net['un_pool_2_2'], filters = 64, kernel_size = [3,3],padding = 'SAME', activation=tf.nn.relu,use_bias=True,bias_initializer=tf.zeros_initializer())
    net['concat2_2'] = tf.compat.v1.concat([net['conv1_2'],net['conv2_2']],3)
        
    net['un_pool_2_3'] = tf.compat.v1.keras.layers.UpSampling2D(size=(2,2),data_format=None,interpolation='nearest')(net['concat2_2'])
    net['conv2_3'] = tf.layers.conv2d(net['un_pool_2_3'], filters = 32, kernel_size = [3,3],padding = 'SAME', activation=tf.nn.relu,use_bias=True,bias_initializer=tf.zeros_initializer())
    net['concat2_3'] = tf.compat.v1.concat([net['conv1_1'],net['conv2_3']],3)

    output = tf.layers.conv2d(net['concat2_3'], filters = 3, kernel_size = [3,3],padding = 'SAME', activation=tf.nn.relu,use_bias=True,bias_initializer=tf.zeros_initializer())
