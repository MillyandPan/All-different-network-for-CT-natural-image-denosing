# -*- coding: utf-8 -*-
"""
Created on Wed Sep 18 11:13:12 2019
...
#    vgg_layers = vgg_raw_net['layers'][0]
    ...
    net['input_channel1'] = tf.compat.v1.placeholder(tf.float32, shape=[batch_size, img_height, img_width, 1])
    net['input_channel2'] = tf.compat.v1.placeholder(tf.float32, shape=[batch_size, img_height, img_width, 1])
    net['input_channel3'] = tf.compat.v1.placeholder(tf.float32, shape=[batch_size, img_height, img_width, 1])
...
#    net['relu3_4'] = relu_layer('relu3_4', net['conv3_4'], b=tf.Variable(tf.ones([128,]),trainable=True,name = 'bias3_4'))
    ...
    net['conv4_2'] = tf.layers.conv2d(net['conv4_1'], filters = 128, kernel_size = [3,3],padding = 'SAME', activation=tf.nn.relu,use_bias=True,bias_initializer=tf.zeros_initializer())
    ...
    output = tf.layers.conv2d(net['conv5_2'], filters = 3, kernel_size = [3,3],padding = 'SAME', activation=tf.nn.relu,use_bias=True,bias_initializer=tf.zeros_initializer())
